<template>
  <div class="report-content">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          ...
          <div class="info-card card">
            ...
            <div class="info-content">
              ...
              <el-button size="default" type="primary" :loading="isLoading" :disabled="isLoading" @click="handleAiAdd">
                重新智能分析
              </el-button>
            </div>
          </div>
            ...
            <div class="metrics-list list">
              <div v-for="(item, index) in coreIndicators" :key="index" class="metric-item item">
                <div class="metric-header header">
                  <el-icon>
                    <component :is="iconMap[item.icon] || Timer" />
                  </el-icon>
                  <span>{{ item.name }}</span>
                  <span class="value">{{ item.value || 0 }}%</span>
                </div>
                <el-progress :percentage="Number(item.value || 0)" :show-text="false" :stroke-width="4"
                  :color="item.color" />
              </div>
            </div>
          </div>
        ...
        <el-col :span="12">
          ...
          <div class="params-card card">
            ...
            <el-table :data="environmentParams" style="width: 100%" :stripe="true" size="small" border>
    ...
    <div v-else class="analyze-button-container">
      ...
      <el-button type="primary" :loading="isLoading" :disabled="isLoading" @click="handleAiAdd">
        开始智能分析
      </el-button>
    </div>
  </div>
...
const getInfo = async () => {
  const res = await AgricultureClassAiReportService.getReport({ classId: props.classId })
  if (res.code === 200) {
...
const getWebSocket = () => {
  ...
  wsStore.registerMessageHandler(WebSocketKey.MQ, WebSocketType.AI_REPORT_KEY, (data) => {
    // 处理AI智能报告消息
    // 使用种类名称（className）或ID作为显示
    const displayName = data.className || String(data.id) || '未知'
    ElNotification({
      ...
      message: `
        <div style="padding: 8px;">
          <div style="font-weight: bold; margin-bottom: 8px; color: #409EFF;">${displayName}</div>
