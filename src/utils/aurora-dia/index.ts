...
export class AuroraDia {
  ...
  activateMotion(): void {
    ...
    if (
      leftEye instanceof HTMLElement &&
...
class AuroraBotSoftware {
          ...
          for (const mouseoverEvents of this.botTips.mouseover) {
            ...
            if (element.matches(selector)) {
              ...
              if (
                ...
                `${selector}-${mouseoverEvents.text[0]}`
              ) {
                return false
              }
              sessionStorage.setItem(
                this.mouseoverEventCacheKey,
                `${selector}-${mouseoverEvents.text[0]}`
              )
              this.showMessage(
                mouseoverEvents.text[
                  Math.floor(Math.random() * mouseoverEvents.text.length)
                ],
                6000,
                9
              )
  ...
  loadLocaleMessages() {
    const locales: Record<string, MessageType> = import.meta.glob('./messages/*.json', {
      eager: true
    })
